...
```mermaid
flowchart LR
    subgraph LibFuzzer[libFuzzer: single process]
        direction TB
        LibCorpus[Corpus]
        LibEngine[libFuzzer engine]
        Harness[LLVMFuzzerTestOneInput]
        TargetFunction[Target function]
        Sanitizer[Sanitizer]

        LibCorpus --> LibEngine
        LibEngine -->|Mutated bytes| Harness
        Harness --> TargetFunction
        TargetFunction -. Coverage feedback .-> LibEngine
        TargetFunction --> Sanitizer
    end

    subgraph AFL[AFL++: external control]
        direction TB
        AflCorpus[Corpus]
        AflEngine[AFL++ engine]
        Input[Input file or stdin]

        subgraph Process[AFL-instrumented program]
            direction TB
            ForkServer[Fork server]
            TargetProcess[Target child process]
            ForkServer -->|fork| TargetProcess
        end

        AflCorpus --> AflEngine
        AflEngine -->|Generate input| Input
        Input --> TargetProcess
        TargetProcess -. Coverage bitmap .-> AflEngine
    end

    LibEngine ~~~ AflEngine
```

libFuzzer 會與 fuzz harness、target function 編譯成同一個 executable，並在同一個 process 中重複呼叫目標函式。這種方式省去了重複啟動 process 與讀寫檔案的成本，但每次執行都會共用同一份 global state。

AFL++ engine 則在 target process 外部產生 input 與管理 corpus。Fork server 會預先完成部分程式初始化，再透過 `fork` 快速建立各次測試使用的 child process。Target 透過 shared-memory coverage bitmap 將執行路徑回傳給 AFL++，因此能保持 process 隔離，又不需要每次都從頭啟動程式。
...
* sanitizer issue：有如下幾種，一般來說 Rust 比較少這類的問題
    ...
    * LSan：memory leak
* Oracle：判斷測試結果是否正確，也可以看成測試的裁判
    * round-trip：decode 和 encode 行為是否一致
    * differential：不同的實作是否有同樣的結果
* Invariant：系統執行前中後絕對不能違反的規則，例如 seq number 不能往後倒退、不能有重複 ID 等等
