...
除了我們想要傳送的最主要的訊息以外，rmw_zenoh 本身也會攜帶其他的資訊，例如序列、發布時間等等。
這些資訊會使用 Zenoh 的 attachment 機制來傳送，這樣就不會影響主要訊息的格式。
這些資訊有什麼呢？

* 8 bytes 的序號 (sequence number)，type 是 int64 且為 little-endian
* 8 bytes 的時間戳 (tim的時間戳)，type 是 int64 且為 little-endian，代表的是從 UNIX EPOCH 開始到現在的 ns
* 1 byte 的 GID 長度 (目前固定為 16)
* 16 bytes 的 GID，type 是 int8

基本上只要發送訊息就一定需要 attachment，所以不管是 publisher、client、service 都會用到。
值得注意的是 service 在回覆 client 的 request 時，sequence number 必須要跟 request 一致。
