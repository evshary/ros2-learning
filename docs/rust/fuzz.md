...
6. 被 merge 並等一段時間之後，用自己綁定的 Google 帳號登入 [oss-fuzz](https://oss-fuzz.com/) 就可以看到運行 Fuzz test 的結果了
    * 我們也可以在 `https://introspector.oss-fuzz.com/project-profile?project=<your_project_name>` 上直接看執行結果（例如 [zenoh](https://introspector.oss-fuzz.com/project-profile?project=zenoh)），這個不需要權限
